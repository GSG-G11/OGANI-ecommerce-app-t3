...
searchForm.onsubmit = (e) => {
	...
	if (searchTerm.trim() !== "") {
		...
		if (neededItems.length > 0) {
			...
			notFoundMessage.textContent = `Sorry, we don't have ${searchTerm}`;
			productsWrapper.innerHTML = "";
			productsWrapper.append(notFoundMessage);
		}
	}
};
...
const getSelectedOption = () => {
	// Get Selected items from dropbox
	productsWrapper.innerHTML = "";
	renderProducts(
		filterProduct(categoryFilter.value, priceFilter.value, availableProducts)
	);
};
